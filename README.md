...
Primary deployment model in this guide:
Single server for AMP + TruBudget
Alternative model is included at the end:
...
3. Initial Access with Username and Password
   ...
   If this is a fresh VM (VMware/VirtualBox/Proxmox/cloud), open VM console and run:
...
This dump file is provided by DG. Copy it from your local machine to the server first (see section 6.3), then restore it.
